    ...
    public class LightButton : MonoBehaviour {
        ...
        void OnTriggerEnter(Collider other) {
            ...
            if (other.name == "LeftHandTriggerCollider" || other.name == "RightHandTriggerCollider") {
                if (isLeft) { if (DoorManager.Data.CanUseLeftLight && !SandboxValues.Data.NoLights) { DoorManager.Data.UseLight(false); } else { RefrenceManager.Data.LeftDoorFailSound.Play(); } }
                else if (!isLeft) { if (DoorManager.Data.CanUseRightLight && !SandboxValues.Data.NoLights) { DoorManager.Data.UseLight(true); } else { RefrenceManager.Data.RightDoorFailSound.Play(); } }
            }
        }
    }
}